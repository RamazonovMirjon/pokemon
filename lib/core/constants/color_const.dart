import 'package:flutter/material.dart';

class ColorConst {
  static Color kPrimaryColor1 = const Color(0XFFFA5AFD);
  static Color kPrimaryColor2 = const Color(0XFFFC7CFF);
  static Color kTextPink = const Color(0XFFF993FB);
  static Color kWhite = const Color(0XFFFFFFFF);
  static Color kTextInPinkContainerColor =
      const Color(0XFFFFFFFF).withOpacity(0.58);
  static Color kGrey = const Color(0XFFE5E5E5);
  static Color kTextGrey = const Color(0XFF838282);
  static Color kIconColor = const Color(0XFF858383);
  static Color kBlack = const Color(0X00000000);
  static Color kTextButtonColor = const Color(0XFFFFFEFC);
  static Color kOrrengeColor = const Color(0XFFFCA600).withOpacity(0.97);
  static Color kBlueColor = const Color(0XFF0083FC).withOpacity(0.46);
}
